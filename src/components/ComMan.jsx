...
const ComMan = () => {
  ...
  const fetchMethods = async () => {
    try {
      const response = await axios.get(
        `https://calendar-server-3.onrender.com/api/communications`
      );
  ...
  const handleSubmit = async () => {
    try {
      if (editId) {
        await axios.put(
          `https://calendar-server-3.onrender.com/api/communications/${editId}`,
          form
        );
        setEditId(null);
      } else {
        await axios.post(`https://calendar-server-3.onrender.com/api/communications`, form);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://calendar-server-3.onrender.com/api/communications/${id}`);
      fetchMethods();
    } catch (error) {
