...
const ComMang = () => {
  ...
  const fetchCompanies = async () => {
    try {
      const response = await axios.get(`https://calendar-server-3.onrender.com/api/companies`);
    ...
    if (validateForm()) {
      try {
        if (currentCompany) {
          // Edit functionality
          await axios.put(
            `https://calendar-server-3.onrender.com/api/companies/edit/${currentCompany._id}`,
            data
          );
        } else {
          // Add functionality
          await axios.post(`https://calendar-server-3.onrender.com/api/companies/add`, data);
        }
        fetchCompanies();
        handleCloseModal();
      } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this company?")) {
      try {
        await axios.delete(`https://calendar-server-3.onrender.com/api/companies/delete/${id}`);
        fetchCompanies();
      } catch (error) {
